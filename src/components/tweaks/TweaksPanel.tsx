...
export function TweaksPanel({ title = 'Tweaks', noDeckControls = false, children }: TweaksPanelProps) {
  ...
  const [railEnabled, setRailEnabled] = useState(() => hasDeckStage && !!(document.querySelector('deck-stage') as unknown as { _railEnabled?: boolean })?._railEnabled);
  ...
  useEffect(() => {
    const onMsg = (e: MessageEvent) => {
      ...
      else if (t === '__omelette_rail_enabled') setRailEnabled(true);
    };
...
export function TweakRow({ label, value, children, inline = false }: {
  label: string; value?: string; children?: React.ReactNode; inline?: boolean;
}) {
  return (
...
export function TweakRadio({ label, value, options, onChange }: {
  ...
  const labelLen = (o: unknown) => String(typeof o === 'object' && o !== null ? (o as { label: string }).label : o).length;
  const maxLen = options.reduce((m, o) => Math.max(m, labelLen(o)), 0);
  const fitsAsSegments = maxLen <= ({ 2: 16, 3: 10 }[options.length] ?? 0);

  if (!fitsAsSegments) {
    const resolve = (s: string) => {
      const m = options.find((o) => String(typeof o === 'object' && o !== null ? (o as { value: string }).value : o) === s);
      return m === undefined ? s : typeof m === 'object' ? (m as { value: string }).value : m;
    };
    return <TweakSelect label={label} value={value} options={options} onChange={(s) => onChange(resolve(s))} />;
  }

  const opts = options.map((o) => (typeof o === 'object' && o !== null ? o as { value: string; label: string } : { value: o, label: o }));
  ...
  return (
    <TweakRow label={label}>
      <select className="twk-field" value={value} onChange={(e) => onChange(e.target.value)}>
        {options.map((o) => {
          const v = typeof o === 'object' && o !== null ? (o as { value: string }).value : o;
          const l = typeof o === 'object' && o !== null ? (o as { label: string }).label : o;
